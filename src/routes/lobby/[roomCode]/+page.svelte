			...
			<div class="rounded-xl bg-white p-8 shadow-[0_4px_12px_rgba(0,0,0,0.06)]">
				{#if mounted}
					...
					>
						...
						>
							0 / {session.esp_teams.length}
						</span>
					</div>
				{/if}

				<div class="grid gap-4">
					{#each session.esp_teams as team, index}
						{#if mounted}
							<div
								data-testid="esp-team-slot"
								class="flex items-center gap-4 rounded-lg border-2 border-[#E5E7EB] bg-[#F9FAFB] p-5 transition-all duration-300"
							...
							>
								...
								<div class="flex-1">
									<div class="mb-1 font-semibold text-[#9CA3AF]">{team.name}</div>
									<div class="text-sm text-[#6B7280]">Waiting for players...</div>
								</div>
								<span
									class="rounded-md bg-[#F3F4F6] px-[0.875rem] py-[0.375rem] text-xs font-semibold text-[#6B7280]"
								>
									Empty
								</span>
							</div>
				...
				{#if mounted}
					...
					>
						...
						<span
							class="ml-auto rounded-full bg-[#F3F4F6] px-4 py-2 text-sm font-semibold text-[#6B7280]"
						>
							0 / {session.destinations.length}
						</span>
					</div>
				{/if}

				<div class="grid gap-4">
					{#each session.destinations as destination, index}
						{#if mounted}
							<div
							...
							>
								...
								<div class="flex-1">
									<div class="mb-1 font-semibold text-[#9CA3AF]">{destination.name}</div>
									<div class="text-sm text-[#6B7280]">Waiting for players...</div>
								</div>
								<span
									class="rounded-md bg-[#F3F4F6] px-[0.875rem] py-[0.375rem] text-xs font-semibold text-[#6B7280]"
